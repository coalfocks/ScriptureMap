...
class GeoDatabase {
    ...
    //
    func volumes() -> [(Int, String)] {
        do {
            let volumes = try dbQueue.inDatabase { (db: Database) -> [(Int, String)] in
                var volumes = [(Int, String)]()
                for row in try Row.fetchAll(db,
                                            "select TOCName, ID from book " +
                                            "where ID < 6 " +
                                            "order by ID") {
                        volumes.append((row["ID"], row["TOCName"]))
                }
                return volumes
            }
            return volumes
    ...
    //
    // Return an array chapters for the given book
    //
    func getChaptersForBook(bookId: Int) -> [Int] {
        do {
            let chapters = try dbQueue.inDatabase { (db: Database) -> [Int] in
                var chapters = [Int]()
                let numChapters = try Int.fetchOne(db,
                                            "select NumChapters from book " +
                                            "where ID = ?",
                                            arguments: [bookId])
                if let chaps = numChapters {
                    for i in 1...chaps {
                        chapters.append(i)
                    }
                } else {
